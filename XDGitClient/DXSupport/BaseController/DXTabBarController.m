...
- (void)configurationTabbarItems
{
    ...
    {
        DXViewController *viewController = (DXViewController *)[self.viewControllers objectAtIndex:i];
        viewController.barTitle = viewController.tabBarItem.title;
        DXTabBarItem *tabItem = [[DXTabBarItem alloc] initWithFrame:CGRectMake(i * width, 0, width, height)];
        tabItem.tag = i;
...
- (void)selectedTabItem:(DXTabBarItem *)item
{
    if (self.currentSelectedIndex != item.tag)
    {
        if (self.currentSelectedIndex > -1) {
            DXTabBarItem *oldItem = [self.itemButtons objectAtIndex:self.currentSelectedIndex];
            oldItem.selected = NO;
        ...
        DXViewController *viewController = (DXViewController *)[self.viewControllers objectAtIndex:item.tag];
        //设置navigationBar
